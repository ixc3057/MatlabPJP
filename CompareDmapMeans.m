...
for iter = 1:length(ROI_name)
    ...
    fnameout =  [base_dir, num2str(patient_number), '\', results_dir1,'\', ROI_name{iter}, '_Fx1_Dmap_linear_array.raw'];
    fid = fopen(fnameout,'r');
    Dmap_Fx1=fread(fid,'double');
    fclose(fid);
    
    % Read fraction distance map
    fnameout =  [base_dir, num2str(patient_number), '\', results_dir2,'\', ROI_name{iter}, '_Fx1_Dmap_linear_array.raw'];
    fid = fopen(fnameout,'r');
